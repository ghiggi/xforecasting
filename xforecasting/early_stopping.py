...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 08:01:13 2022
...
class EarlyStopping:
    ...
    def __call__(self, training_info):
        ...
        iteration_from_last_ar_update = getattr(
            training_info, "iteration_from_last_ar_update"
        )
        if not self.disabled:
            if iteration_from_last_ar_update > self.minimum_iterations:
                if self.best_score is None:
                    ...
                    ((self.best_score - score) < self.minimum_improvement)
                    and (self.mode == "min")
                ) or (
                    (((score - self.best_score) < self.minimum_improvement))
                    and (self.mode == "max")
                ):
                    self.counter += 1
