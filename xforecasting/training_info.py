...
def plot_loss(
    ...
    if training_loss is None and validation_loss is None:
        raise ValueError(
            "At least one between 'training_loss' and 'validation_loss' must provided."
        )
    ##------------------------------------------------------------------------.
    ...
    if validation_loss is not None:
        ...
        if len(iterations) != len(validation_loss):
            raise ValueError(
                "'iterations' and 'validation_loss' must have same length."
            )
...
class AR_TrainingInfo:
    ...
    def __init__(self, ar_iterations, epochs, ar_scheduler):
        ...
        self.start_epochs_iterations = (
            []
        )  # list the iterations numbers when a new epoch start
    ...
    def update_training_stats(
        self, total_loss, dict_loss_per_ar_iteration, ar_scheduler, lr_scheduler=None
    ):
        ...
        for i in range(current_ar_iterations + 1):
            ...
            self.training_loss_per_ar_iteration[i]["loss"].append(
                dict_loss_per_ar_iteration[i].item()
            )

        # Update AR weights
        for i in range(current_ar_iterations + 1):
            self.ar_weights_per_ar_iteration[i]["iteration"].append(self.iteration)
            self.ar_weights_per_ar_iteration[i]["ar_absolute_weights"].append(
                ar_scheduler.ar_absolute_weights[i]
            )
            self.ar_weights_per_ar_iteration[i]["ar_weights"].append(
                ar_scheduler.ar_weights[i]
            )
    ...
    def update_validation_stats(self, total_loss, dict_loss_per_ar_iteration):
        ...
        for i in range(current_ar_iterations + 1):
            ...
            self.validation_loss_per_ar_iteration[i]["loss"].append(
                dict_loss_per_ar_iteration[i].item()
            )
    ...
    def print_epoch_info(self):
        ...
        avg_training_loss = np.mean(
            self.training_total_loss[self.start_epoch_scoring_idx :]
        )
        print(" ")
        print(
            "- Epoch: {epoch:3d}/{n_epoch:3d}".format(
                epoch=self.epoch, n_epoch=self.n_epochs
            )
        )
        print(
            "- Training loss: {training_total_loss:.3f}".format(
                training_total_loss=avg_training_loss
            )
        )
        # If validation data are provided
        if len(self.validation_total_loss) != 0:
            avg_validation_loss = np.mean(
                self.validation_total_loss[self.start_epoch_scoring_idx :]
            )
            print(
                "- Validation Loss: {validation_total_loss:.3f}".format(
                    validation_total_loss=avg_validation_loss
                )
            )
        print(
            "- Elapsed time: {elapsed_time:.2f} minutes".format(
                elapsed_time=(time.time() - self.epoch_time_start) / 60
            )
        )
    ...
    def iterations_of_ar_updates(self):
        ...
        iter_ar_update = [
            self.ar_weights_per_ar_iteration[i]["iteration"][0]
    ...
    ##------------------------------------------------------------------------.
    def plot_loss_per_ar_iteration(
        self,
        ar_iteration,
        title=None,
        plot_training=True,
        ...
        if (not plot_validation) and (not plot_training):
            raise ValueError(
                "At least one between 'plot_training' and 'plot_validation' must be True."
            )
        ##--------------------------------------------------------------------.
        # Check ar_iteration
        ...
        if ar_iteration > self.ar_iterations:
            raise ValueError(
                "The  maximum 'ar_iteration' is {}.".format(self.ar_iterations)
            )
        ##--------------------------------------------------------------------.
        # Create figure if ax not provided
        ...
        if add_ar_weights_updates:
            ...
            if len(iterations_of_ar_updates) > 0:
                [
                    ax.axvline(x=x, color=(0, 0, 0, 0.90), linewidth=0.1)
                    for x in iterations_of_ar_updates
                ]
        ##--------------------------------------------------------------------.
    ...
    def plot_ar_weights(self, normalized=True, xlim=None, ylim=(0, 1.05), ax=None):
        ...
        if not normalized:
            for i in range(self.ar_iterations + 1):
                ax.plot(
                    self.ar_weights_per_ar_iteration[i]["iteration"],
        ...
        ## Add legend
        ax.legend(
            labels=list(range(self.ar_iterations + 1)),
            title="AR iteration",
            loc="lower left",
        )
        ##--------------------------------------------------------------------.
    ...
    def plots(self, model_dir=None, ylim=(0, 0.06)):
        ...
        for ar_iteration in range(self.ar_iterations + 1):
            ax = self.plot_loss_per_ar_iteration(
        ...
        if model_dir is not None:
            fig.savefig(
                os.path.join(
                    model_dir, "figs/training_info/Loss_at_all_ar_iterations.png"
                )
            )
        else:
            plt.show()
        ...
        for ar_iteration in range(self.ar_iterations + 1):
            fig = self.plot_loss_per_ar_iteration(
                ar_iteration=ar_iteration,
                linewidth=0.6,
                ylim=ylim,
            ...
            if model_dir is not None:
                fname = os.path.join(
                    model_dir,
                    "figs/training_info/Loss_at_ar_{}.png".format(ar_iteration),
                )
                fig.savefig(fname)
            else:
                plt.show()
        ...
        if model_dir is not None:
            fig.savefig(
                os.path.join(model_dir, "figs/training_info/ar_Normalized_Weights.png")
            )
        else:
            plt.show()
        ### - Plot absolute AR weights
        ...
        if model_dir is not None:
            fig.savefig(
                os.path.join(model_dir, "figs/training_info/ar_Absolute_Weights.png")
            )
        else:
            plt.show()
        ##--------------------------------------------------------------------.
