...
import xarray as xr
import warnings
from .xr import xr_have_Dataset_vars_same_dims
...
def check_timesteps_format(timesteps):
    ...
    if timesteps is None:
        return None
    if not isinstance(
        timesteps, (str, list, np.ndarray, datetime.datetime, np.datetime64)
    ):
        raise ValueError(
            "The timestep(s) must be provided as a str, list, np.array of datetime or np.datetime64."
        )
    ...
    if isinstance(timesteps, np.ndarray):
        if (
            len(timesteps.shape) == 0
        ):  # array('2019-01-01', dtype='datetime64) --> fail len(timesteps)
...
def _check_has_feature_dimension(data, feature_dim="feature"):
    if feature_dim not in data.dims:
        data_type = type(data)
        raise ValueError(f"The '{data_type}' must have a dimension called 'feature'.")
    return None
...
def _check_has_time_dimension(data, time_dim="time"):
    if time_dim not in data.dims:
        data_type = type(data)
        raise ValueError(f"The '{data_type}' must have a dimension called 'time'.")
    return None


def _check_temporal_data(
    data, data_type, feature_dim="feature", time_dim="time", verbose=False
):
    if data is None:
        return None
    # - Checks data type
    ...
    if isinstance(data, xr.Dataset):
        if not xr_have_Dataset_vars_same_dims(data):
            raise ValueError(
                "All xr.DataArray(s) within the {!r} xr.Dataset must have the same dimensions.".format(
                    data_type
                )
            )
    ...
    if len(dims_optional) < 1:
        raise ValueError(
            "{!r} must have at least one additional dimension (i.e. space) other than {!r}".format(
                data_type, dims_required
            )
        )
    return None
...
def _check_static_data(data, data_type, feature_dim="feature", time_dim="time"):
    ...
    if time_dim in list(data.dims):
        raise ValueError(
            "{!r} must not contain the 'time' dimension.".format(data_type)
        )
    # - If Dataset, check that all DataArray have same dimensions
    if isinstance(data, xr.Dataset):
        if not xr_have_Dataset_vars_same_dims(data):
...
def _get_subset_timesteps_idxs(timesteps, subset_timesteps, strict_match=True):
    ...
    subset_idxs = np.array(
        [idx for idx, v in enumerate(timesteps) if v in set(subset_timesteps)]
    )
    # If no idxs available,means that subset_timesteps are not in the period covered by 'data_dyamic
    if subset_idxs.size == 0:
        raise ValueError(
            "All 'forecast_reference_times' are not within the time period covered by 'data_dynamic'."
        )
    if len(subset_idxs) != len(subset_timesteps):
        timesteps_not_in = subset_timesteps[
            np.isin(subset_timesteps, timesteps, invert=True)
        ]
        if strict_match:
            raise ValueError(
                "The following 'forecast_start_time(s)' are not within the time period covered by 'data_dynamic': {}".format(
                    list(timesteps_not_in)
                )
            )
        else:
            warnings.warn(
...
def get_ar_model_tensor_info(
    ar_settings, data_dynamic, data_static=None, data_bc=None, bc_generator=None
):
    ...
    if not np.all(np.isin(["input_k", "output_k"], ar_settings_keys)):
        raise ValueError(
            "The 'ar_settings' dictionary must contain 'input_k' and 'output_k' keys."
        )
    ##------------------------------------------------------------------------.
    ...
    if bc_generator is not None:
        ...
        raise NotImplementedError()
    _check_temporal_data(
        data_bc, data_type="data_bc", feature_dim=feature_dim, time_dim=time_dim
    )

    ##------------------------------------------------------------------------.
    ### Checks data_static
    _check_static_data(
        data_static, data_type="data_statuc", feature_dim=feature_dim, time_dim=time_dim
    )
    ...
    feature_dynamic = (
        feature_order["dynamic"] if feature_order["dynamic"] is not None else []
    )
    feature_bc = feature_order["bc"] if feature_order["bc"] is not None else []
    feature_static = (
        feature_order["static"] if feature_order["static"] is not None else []
    )
    ...
    for data_type in ["dynamic", "bc", "static"]:
        if dim_order[data_type] is not None:
            input_shape_info[data_type] = {
                k: v for k, v in zip(dim_order[data_type], shape_order[data_type])
            }
            if data_type != "static":
    ...
    input_shape = [
        v if k != "feature" else input_n_feature
        for k, v in input_shape_info["dynamic"].items()
    ]
    output_shape = [
        v if k != "feature" else output_n_feature
        for k, v in output_shape_info["dynamic"].items()
    ]
    ##------------------------------------------------------------------------.
