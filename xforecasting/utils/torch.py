...
def check_device(device):
    ...
    if not isinstance(device, torch.device):
        raise TypeError(
            "Specify device as torch.device or as string : 'cpu','cuda',..."
        )
    if device.type != "cpu":
        if not torch.cuda.is_available():
            raise ValueError(
                "Not possible to set up a cuda device. CUDA is not available!"
            )
    return device
...
def check_pin_memory(pin_memory, num_workers, device):
    ...
    if not isinstance(pin_memory, bool):
        raise TypeError(
            "'pin_memory' must be either True or False. If num_workers > 0, set to False ;) "
        )
    # CPU case
    if device.type == "cpu":
        if pin_memory:
...
def check_prefetch_in_gpu(prefetch_in_gpu, num_workers, device):
    ...
    if not isinstance(prefetch_in_gpu, bool):
        raise TypeError(
            "'prefetch_in_gpu' must be either True or False. If num_workers > 0, set to False ;) "
        )
    # CPU case
    if device.type == "cpu":
        if prefetch_in_gpu:
    ...
    # GPU case with multiprocess
        ...
        print(
            "- Prefetch in GPU with multiprocessing is currently unstable.\n\
...
def _generate_forward_hook(handle, summary_forward, m_key):
    def forward_hook(module, input, output):
        ...
        if device.type != "cpu":
            summary_forward[m_key]["memory_allocated"] = (
                torch.cuda.memory_allocated() / 1000 / 1000
            )
            summary_forward[m_key]["memory_cached"] = (
                torch.cuda.memory_cached() / 1000 / 1000
            )
        ...
        if len(input) > 1:
            summary_forward[m_key]["input_shape"] = [
                list(i.size()) if i is not None else None for i in input
            ]
        ...
        if isinstance(output, (list, tuple)):
            summary_forward[m_key]["output_shape"] = [
                list(o.size()) if o is not None else None for o in output
            ]
        else:
...
def _generate_forward_pre_hook(handle, summary_pre_forward, m_key):
    def forward_pre_hook(module, input):
        ...
        # Measure memory allocation
        if device.type != "cpu":
            summary_pre_forward[m_key]["memory_allocated"] = (
                torch.cuda.memory_allocated() / 1000 / 1000
            )
            summary_pre_forward[m_key]["memory_cached"] = (
                torch.cuda.memory_cached() / 1000 / 1000
            )
...
def profile_layers(
    model, input_size, batch_size=32, dtypes=None, device=torch.device("cpu")
):
    ...
    x = [
        torch.rand(batch_size, *in_size, dtype=dtype, device=device)
    ...
    for idx, module in enumerate(model.modules()):
        if not isinstance(module, (torch.nn.Sequential, torch.nn.ModuleList)):
            ...
            h = module.register_forward_pre_hook(
                _generate_forward_pre_hook(
                    hooks, summary_pre_forward=summary_pre_forward, m_key=m_key
                )
            )
            hooks.append(h)
            h = module.register_forward_hook(
                _generate_forward_hook(
                    hooks, summary_forward=summary_forward, m_key=m_key
                )
            )
            hooks.append(h)
    ...
    for key in summary.keys():
        t_f = summary_forward[key]["time"]
        t_i = summary_pre_forward[key]["time"]
        summary[key]["time"] = t_f - t_i
        if device.type != "cpu":
            mem_f = summary_forward[key]["memory_allocated"]
            mem_i = summary_pre_forward[key]["memory_allocated"]
            summary[key]["delta_memory_allocated"] = mem_f - mem_i
            mem_f = summary_forward[key]["memory_cached"]
            mem_i = summary_pre_forward[key]["memory_cached"]
            summary[key]["delta_memory_cached"] = mem_f - mem_i
    ...
    df = (
        df.dropna()
    )  # Remove repeated info (i.e. ConvBlock) ...but discard also pooling
    ...
    if device.type == "cpu":
        ...
        df["delta_memory_allocated"] = df["delta_memory_allocated"].apply(
            lambda x: round(x, 2)
        )
        df["delta_memory_cached"] = df["delta_memory_cached"].apply(
            lambda x: round(x, 2)
        )
...
def summarize_model(
    ...
    tmp_model.to(device)
    table, summary_str, summary = profile_layers(
        model=tmp_model,
        input_size=input_size,
...
##----------------------------------------------------------------------------.
def profile_model(
    model, input_size, batch_size, device="cpu", dtypes=None, row_limit=10
):
