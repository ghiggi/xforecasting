...
def get_dict_X_dynamic(ar_iterations, forecast_cycle, input_k):
    ...
    for i in range(ar_iterations + 1):
        idxs = get_idx_lag(
            idx_start=0, ar_iteration=i, forecast_cycle=forecast_cycle, input_k=input_k
        )
...
def get_dict_X_bc(ar_iterations, forecast_cycle, input_k):
    ...
    dict_X_bc = {}
    for i in range(ar_iterations + 1):
        dict_X_bc[i] = get_idx_lag(
            idx_start=0, ar_iteration=i, forecast_cycle=forecast_cycle, input_k=input_k
        )
    return dict_X_bc


def get_dict_stack_info(
    ar_iterations, forecast_cycle, input_k, output_k, stack_most_recent_prediction=True
):
    ...
    try:
        for i, idx_X_future in dict_X_future.items():
            if idx_X_future is None:
                ...
                for idx in idx_X_future:
                    ...
                    l_solutions = [
                        (j, np.argwhere(dict_Y[j] == idx).tolist()[0][0])
                        for j in range(i)
                        if idx in dict_Y[j]
                    ]
        ...
        raise ValueError(
            "Review the AR settings. AR training is not possible with the current configuration!"
        )
    ##------------------------------------------------------------------------.
    ...
    for i in range(ar_iterations + 1)[::-1]:
        ...
        if l_tuple is None:
            ...
            leadtime_arr = leadtime_arr[
                np.isin(leadtime_arr, idx_arr_removed, invert=True)
            ]
...
def check_input_k(input_k, ar_iterations):
    ...
    if ar_iterations > 0:
        if np.any(input_k == 0):
            raise ValueError(
                "input_k contains values equal to 0. Past timesteps must be specified with negative values"
            )
        if np.all(input_k > 0):
...
def check_output_k(output_k):
    ...
    if not np.any(output_k == 0):
        raise ValueError(
            "output_k must start with a 0 value. 0 indicates the 'current' timestep to predict."
        )
    return output_k
...
def check_ar_iterations(ar_iterations):
    ...
    if ar_iterations >= 1:
        print(
            " - Autoregressive training with %d AR iterations --> Specified."
            % ar_iterations
        )
...
def check_ar_settings(
    ...
    forecast_cycle = check_forecast_cycle(
        forecast_cycle=forecast_cycle, ar_iterations=ar_iterations
    )
    ##------------------------------------------------------------------------.
    # Check autoregressive training is feasible
    if ar_iterations >= 1:
        dict_Y_to_stack, _ = get_dict_stack_info(
...
##############################
def _arr_window_info(
    input_k, output_k, forecast_cycle, ar_iterations, past_margin=0, future_margin=0
):
    ...
    future_idxs = future_margin + get_last_valid_idx(
...
def plot_ar_settings(
    ...
    idx_start, width, height = _arr_window_info(
        input_k=input_k,
        output_k=output_k,
        forecast_cycle=forecast_cycle,
        ar_iterations=ar_iterations,
    ...
    for i in range(height):
        idxs_lag = get_idx_lag(
            idx_start=idx_start,
            ar_iteration=i,
            forecast_cycle=forecast_cycle,
            input_k=input_k,
        )
        idxs_forecasted = get_idx_forecast(
            idx_start=idx_start,
            ar_iteration=i,
            forecast_cycle=forecast_cycle,
            output_k=output_k,
    ...
    if (ar_iterations >= 1) and hatch:
        ...
        ax.pcolor(
            np.arange(width + 1) - 0.5,
            np.arange(height + 1) - 0.5,
            hatch_arr,
            hatch="//",
            alpha=0.0,
        )
