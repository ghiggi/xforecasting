    ...
    if isinstance(chunks, dict):
        ...
        if not CHUNKS_PER_VARIABLE and not CHUNKS_DIMS:
            if np.any(np.isin(list(chunks.keys()), dim_names)):
                print(
                    "The 'chunks' dictionary must contain the keys {}".format(dim_names)
                )
                raise ValueError(
                    "Please specify specific chunks for each Dataset dimension."
                )
            if np.any(np.isin(list(chunks.keys()), variable_names)):
                print(
                    "The 'chunks' dictionary must contain the keys {}".format(
                        variable_names
                    )
                )
                raise ValueError(
                    "Please specify specific chunks for each Dataset variable."
                )
    if isinstance(default_chunks, dict):
        DEFAULT_CHUNKS_PER_VARIABLE = np.all(
            np.isin(list(default_chunks.keys()), variable_names)
        )
        ...
        if not DEFAULT_CHUNKS_PER_VARIABLE and not DEFAULT_CHUNKS_DIMS:
            if np.any(np.isin(list(default_chunks.keys()), dim_names)):
                raise ValueError(
                    "Please specify specific default_chunks for each Dataset dimension."
                )
            if np.any(np.isin(list(default_chunks.keys()), variable_names)):
                raise ValueError(
                    "Please specify specific default_chunks for each Dataset variable."
                )
    ##------------------------------------------------------------------------.
    # If chunks = "auto"
    if isinstance(chunks, str):
