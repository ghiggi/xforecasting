...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  2 15:39:07 2022
...
def xr_have_Dataset_vars_same_dims(ds):
    if not isinstance(ds, xr.Dataset):
        raise TypeError(
            "Expecting an xr.Dataset within xr_have_Dataset_vars_same_dims()."
        )
    unordered_dims = list(
        ds.dims
    )  # This does not correspond to dims of DataArrays !!!!
    ...
    missing_dim = np.array(unordered_dims)[
        np.isin(unordered_dims, dims, invert=True)
    ].tolist()
    if len(missing_dim) >= 1:
        print(
            "The Dataset variable {!r} does not have dimensions {!r}.".format(
                variables[0], missing_dim
            )
        )
    for var in variables:
        ...
        if not np.array_equal(da_dims, dims):
            ...
            if len(missing_dim) >= 1:
                ...
                print(
                    "The Dataset variable {!r} have dimension {!r} instead of {!r}.".format(
                        var, da_dims, dims
                    )
                )
            return False
