...
# ---> Can become a method to try when applying criterion no?
#
####---------------------------------------------------------------------------.
# ############################
#### Autotune num_workers ####
# ############################
def timing_AR_Training(
    dataset,
    model,
    optimizer,
    criterion,
        ...
        for count in range(n_repetitions):
            ...
            for ar_iteration in range(ar_iterations + 1):
                ...
                if device.type != "cpu" and ar_iteration == 0:
                    batch_memory_size = (
                        torch.cuda.memory_allocated() / 1000 / 1000 - model_params_size
                    )
                ##-----------------------------------------------------------------.
                ...
                if ar_training_strategy == "AR":
                    ...
                    t_i = get_time()
                    dict_training_loss_per_ar_iteration[ar_iteration] = (
                        dict_training_loss_per_ar_iteration[ar_iteration]
                        * ar_scheduler.ar_weights[ar_iteration]
                    )
                    tmp_ar_loss_timing = tmp_ar_loss_timing + (get_time() - t_i)
                    # - Measure model size requirements
                    if device.type != "cpu":
                        model_memory_allocation = (
                    ...
                    if training_mode:
                        ...
                        tmp_ar_backprop_timing = tmp_ar_backprop_timing + (
                            get_time() - t_i
                        )
                    # - Update the total (AR weighted) loss
                    t_i = get_time()
                    if ar_iteration == 0:
                        training_total_loss = dict_training_loss_per_ar_iteration[
                            ar_iteration
                        ]
                    else:
                        training_total_loss += dict_training_loss_per_ar_iteration[
                ...
                tmp_ar_data_removal_timing = tmp_ar_data_removal_timing + (
                    get_time() - t_i
                )
    ...
    ##-------------------------------------------------------------------------.
    memory_info = {
        "Model parameters": model_params_size,
    ...
    if verbose:
        ...
        if device.type != "cpu":
            print(
                "- Model parameters requires {:.2f} MB in GPU".format(
                    memory_info["Model parameters"]
                )
            )
            print(
                "- A batch with {} samples for {} AR iterations allocate {:.2f} MB in GPU".format(
                    batch_size, ar_iterations, memory_info["Batch"]
                )
            )
            print(
                "- The model forward pass allocates {:.2f} MB in GPU.".format(
                    memory_info["Forward pass"]
                )
            )
    ##------------------------------------------------------------------------.
...
def tune_num_workers(
    ...
    for num_workers in num_workers_list:
        timing_info, memory_info = timing_AR_Training(
            dataset=dataset,
            model=model,
            optimizer=optimizer,
            criterion=criterion,
    ...
    if verbose:
        ...
        if dataset.device.type != "cpu":
            ...
            print(
                "- A batch with {} samples for {} AR iterations allocate {:.2f} MB in GPU.".format(
                    batch_size, dataset.ar_iterations, memory_info["Batch"]
                )
            )
            print(
                "- The model forward pass allocates {:.2f} MB in GPU.".format(
    ...
    with dask.config.set(scheduler="synchronous"):
        ...
        if ar_iterations == 0:
            if ar_scheduler.method != "constant":
                print(
                    "Since 'ar_iterations' is 0, ar_scheduler 'method' is changed to 'constant'."
                )
        ...
        if validation_data_dynamic is not None:
            if not xr_is_aligned(
                training_data_dynamic, validation_data_dynamic, exclude="time"
            ):
                raise ValueError(
                    "training_data_dynamic' and 'validation_data_dynamic' does not"
                    "share same dimensions (order and values)(excluding 'time')."
                )
        if validation_data_bc is not None:
            if training_data_dynamic is None:
                raise ValueError(
                    "If 'validation_data_bc' is provided, also 'training_data_dynamic' must be specified."
                )
            if not xr_is_aligned(training_data_bc, validation_data_bc, exclude="time"):
                raise ValueError(
                    "training_data_bc' and 'validation_data_bc' does not"
                    "share same dimensions (order and values)(excluding 'time')."
                )
        ...
        if validation_data_dynamic is not None:
            validationDataset = AutoregressiveDataset(
        ...
        # - Time AR training
        print(
            "- Timing AR training with {} AR iterations:".format(
                trainingDataset.ar_iterations
            )
        )
        training_num_workers = tune_num_workers(
        ...
        print(
            "  --> Selecting num_workers={} for TrainingDataLoader.".format(
        ...
        if validationDataset is not None:
            print()
            print(
                "- Timing AR validation with {} AR iterations:".format(
                    validationDataset.ar_iterations
                )
            )
            validation_num_workers = tune_num_workers(
            ...
            print(
                "  --> Selecting num_workers={} for ValidationDataLoader.".format(
        ...
        if validation_data_dynamic is not None:
            validationDataLoader = AutoregressiveDataLoader(
                dataset=validationDataset,
                batch_size=validation_batch_size,
                drop_last_batch=drop_last_batch,
                shuffle=shuffle,
        ...
        for epoch in range(epochs):
            ...
            for batch_count in batch_indices:
                ...
                if validationDataset is None:
                    ...
                    if (
                        dict_training_loss_per_ar_iteration[0].item() > 10000
                    ):  # TODO: This require CPU-GPU synchronization
                    ...
                    if ar_training_info.iteration_from_last_scoring == scoring_interval:
                        ...
                        with torch.set_grad_enabled(False):
                            ...
                            for ar_iteration in range(
                                ...
                                ##------------------------------------------------.
                                # Forward pass and store output for stacking into next AR iterations
                                dict_validation_Y_predicted[ar_iteration] = (
                                    swag_model(torch_X)
                                    if swag_training
                                    else model(torch_X)
                                )
                                ...
                                remove_unused_Y(
                                    ar_iteration=ar_iteration,
                                    dict_Y_predicted=dict_validation_Y_predicted,
                                    dict_Y_to_remove=validation_batch_dict[
                                        "dict_Y_to_remove"
                                    ],
                                )
                                del Y_pred, Y_obs, torch_X, torch_Y
                ...
                if ar_training_info.iteration_from_last_scoring == scoring_interval:
                    ...
                    if early_stopping is not None and early_stopping(ar_training_info):
                        ...
                        if ar_scheduler.current_ar_iterations < ar_iterations:
                            ...
                            print(
                                "- Updating training to {} AR iterations {}.".format(
                            ...
                            del (
                                trainingDataLoader,
                                trainingDataLoader_iter,
                            )  # to avoid deadlocks
                            trainingDataset.update_ar_iterations(
                                ar_scheduler.current_ar_iterations
                            )
                            # - Validation
                            if validationDataset is not None:
                                del (
                                    ...
                                    validationDataLoader_iter,
                                )  # to avoid deadlocks
                                validationDataset.update_ar_iterations(
                                    ar_scheduler.current_ar_iterations
                                )
                            ##----------------------------------------------------.
                            ## Time execution
                            ...
                            if validationDataset is not None:
                                ...
                                validationDataLoader_iter = cylic_iterator(
                                    validationDataLoader
                                )
            ...
            if save_model_each_epoch:
                model_weights = (
                    swag_model.state_dict() if swag_training else model.state_dict()
                )
                torch.save(
                    model_weights, model_fpath[:-3] + "_epoch_{}".format(epoch) + ".h5"
                )
        ...
        print(
            "- Total elapsed time: {:.2f} hours.".format(
                (time.time() - time_start_training) / 60 / 60
            )
        )
        print("- Saving model to {}".format(model_fpath))
        model_weights = (
            swag_model.state_dict() if (swag and swag_model) else model.state_dict()
        )
        ...
        with open(
            os.path.join(os.path.dirname(model_fpath), "AR_TrainingInfo.pickle"), "wb"
        ) as handle:
