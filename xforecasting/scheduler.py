...
class AR_Scheduler:
    ...
    def __init__(
        ...
        if initial_ar_weights is not None and initial_ar_absolute_weights is not None:
            raise ValueError(
                "Specify either 'initial_ar_weights' or 'initial_ar_absolute_weights'."
            )
        ...
        # Check initial_ar_absolute_weights
            ...
            initial_ar_absolute_weights = (
                initial_ar_absolute_weights / initial_ar_absolute_weights.max()
            )
            # Compute the normalized AR weights
            initial_ar_weights = (
                initial_ar_absolute_weights / initial_ar_absolute_weights.sum()
            )
        else:
        ...
        if step_interval is not None:
            if step_interval <= 0:
                raise ValueError(
                    "'step_interval' must be an integer value equal or larger than 1."
                )
        ##---------------------------------------------------------------------.
        ...
        if method in ["StepwiseStep", "HalfStep"]:
            if step_interval is None:
                raise ValueError(
                    "'{}' method requires specification of the 'step_interval' argument".format(
                        method
                    )
                )
        if method in ["HalfStep", "StepwiseStep", "LinearStep", "ExponentialStep"]:
            if factor is None:
                raise ValueError(
                    "'{}' method requires specification of the 'factor' argument".format(
                        method
                    )
                )
    ...
    def step(self):
        """Update AR weights."""
        # Update step count
        self.temporary_step_count = (
            self.temporary_step_count + 1
        )  # for 'StepwiseDecay' and 'HalfDecay'
        self.global_step_count_arr = (
            self.global_step_count_arr + 1
        )  # for 'LinearDecay' and 'ExponentialDecay'
        ##---------------------------------------------------------------------.
        if self.current_ar_iterations > 0:
            ...
            if self.fixed_ar_weights is not None:
                tmp_fixed_ar_weights = self.fixed_ar_weights[
                    self.fixed_ar_weights < self.current_ar_iterations
                ]
                self.ar_absolute_weights[
                    tmp_fixed_ar_weights
                ] = self.ar_absolute_initial_weights[tmp_fixed_ar_weights]
            ##---------------------------------------------------------------------.
            # Retrieve normalized AR weights (summing up to 1)
            self.ar_weights = np.array(self.ar_absolute_weights) / np.sum(
                self.ar_absolute_weights
            )
    ...
    def update(self):
        ...
        if not self.smooth_growth:  # ... with (absolute) value 1
            ...
            self.ar_absolute_initial_weights = np.append(
                self.ar_absolute_initial_weights, 1
            )
            ...
            # Add new weight
            self.ar_absolute_initial_weights = np.append(
                self.ar_absolute_initial_weights, 0
            )
        ...
        self.global_step_count_arr[
            -1
        ] = 0  # Reset the last (because will start to decay)
        self.global_step_count_arr = np.append(self.global_step_count_arr, 0)


# ----------------------------------------------------------------------------.
def plot_AR_scheduler(
    ar_scheduler,
    n_updates=4,
    update_every=15,
    ...
    for u in range(n_updates + 1):
        for i in range(update_every + 1):
            ...
            for ar_iteration in range(current_ar_iterations + 1):
                ...
                ar_weights_per_ar_iteration[ar_iteration]["ar_absolute_weights"].append(
                    ar_scheduler.ar_absolute_weights[ar_iteration]
                )
                ar_weights_per_ar_iteration[ar_iteration]["ar_weights"].append(
    ...
    if plot_absolute_ar_weights:
        ...
        for ar_iteration in range(n_final_ar_weights + 1):
            plt.plot(
                ar_weights_per_ar_iteration[ar_iteration]["iteration"],
