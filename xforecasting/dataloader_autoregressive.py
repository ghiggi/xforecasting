...
def _check_ar_batch_fun_kwargs(ar_batch_fun):
    ...
    missing_kwargs = np.array(required_kwargs)[
        np.isin(required_kwargs, fun_kwargs, invert=True)
    ]
    excess_kwargs = np.array(fun_kwargs)[
        np.isin(fun_kwargs, required_kwargs, invert=True)
    ]
    if len(missing_kwargs) > 0:
        raise ValueError("ar_batch_fun miss the {!r] kwargs.".format(missing_kwargs))
    if len(excess_kwargs) > 0:
        raise ValueError(
            "ar_batch_fun does not require the {!r] kwargs.".format(excess_kwargs)
        )
    return None


def get_aligned_ar_batch(
    ar_iteration,
    batch_dict,
    dict_Y_predicted,
    device="cpu",
    ...
    if data_availability["bc"]:
        if not np.array_equal(dim_order_dynamic, dim_order_bc):
            raise ValueError(
                "'dynamic and bc batch data do not have the same dimension order!"
            )
    ##------------------------------------------------------------------------.
    ...
    if dict_X_dynamic_batched[ar_iteration] is not None:
        if not prefetched_in_gpu:
            torch_X = dict_X_dynamic_batched[ar_iteration].to(
    ...
    if bc_is_available:
        if not prefetched_in_gpu:
            torch_X_bc = dict_X_bc_batched[ar_iteration].to(
                device=device, non_blocking=asyncronous_gpu_transfer
            )
        else:
    ...
    if training_mode:
        if not prefetched_in_gpu:
            torch_Y = dict_Y_batched[ar_iteration].to(
                device=device, non_blocking=asyncronous_gpu_transfer
            )
        else:
    ...
    if bc_is_available:
        ...
        if not np.allclose(shape_dynamic, shape_bc):
            raise ValueError(
                "'data_dynamic' and 'data_bc' are likely disaligned."
    ...
    if static_is_available:
        ...
        if not np.allclose(shape_dynamic, shape_static):
            raise ValueError(
                "'data_dynamic' and 'data_static' are likely disaligned."
                "You might want to use the 'ar_batch_fun' like 'get_disaligned_ar_batch'."
                "'Excluding the feature dimension, the static batch has shape {},"
                " while the dynamic batch has shape {}".format(
                    shape_dynamic, shape_static
                )
            )
...
class AutoregressiveDataset(Dataset):
    ...
    def __init__(
        ...
        if not xr_has_uniform_resolution(data_dynamic, dim="time"):
            raise ValueError(
                "'data_dynamic' does not have an uniform 'time' resolution."
            )
        ...
        if self.training_mode:
            ...
            for i in range(self.ar_iterations + 1):
                dict_Y_data[i] = torch.as_tensor(
                    torch.from_numpy(
                        da_dynamic_subset.sel(rel_idx=self.dict_rel_idx_Y[i]).values
                    ),
                    dtype=self.torch_dtype,
                    device="cpu",
                )
    ...
    for count in range(n_repetitions):
        ...
        for i in range(ar_iterations + 1):
            ...
            if device.type != "cpu" and i == 0:
                batch_memory_size = (
                    torch.cuda.memory_allocated() / 1000 / 1000 - background_memory
                )
            ##------------------------------------------------------------.
