...
def check_timedelta_unit(timedelta_unit):
    ...
    if timedelta_unit not in valid_timedelta_unit:
        raise ValueError(
            "Specify a valid 'timedelta_unit': {}".format(valid_timedelta_unit)
        )
...
def get_dict_Y_pred_selection(
    dim_info, dict_forecast_rel_idx_Y, keep_first_prediction=True
):
    ...
    if keep_first_prediction:
        dict_k_selection = {
            leadtime: min(dict_k_occurence[leadtime])
            for leadtime in dict_k_occurence.keys()
        }
    else:
        dict_k_selection = {
            leadtime: max(dict_k_occurence[leadtime])
            for leadtime in dict_k_occurence.keys()
        }
    # Build {ar_iteration: [(leadtime, subset_indexing), (...,...)]}
    dict_Y_pred_selection = {
        ar_iteration: [] for ar_iteration in range(ar_iterations + 1)
    }
    for leadtime, ar_iteration in dict_k_selection.items():
        # Retrieve tuple (leadtime, Y_tensor_indexing)
        leadtime_slice_idx = np.argwhere(
            dict_forecast_rel_idx_Y[ar_iteration] == leadtime
        )[0][0]
...
def create_ds_forecast(
    ...
    # - Create DataArray
    forecast_dims = (
        ["forecast_reference_time", "leadtime"] + dims_optional + ["feature"]
    )
    da = xr.DataArray(
        Y_forecasts,
        dims=forecast_dims,
...
def rescale_forecasts(ds, scaler, reconcat=True):
    ...
    for i in range(len(ds["forecast_reference_time"])):
        ...
        l_rescaled_ds.append(
            scaler.inverse_transform(tmp_ds)
            .swap_dims({"time": "leadtime"})
            .drop("time")
        )
    if reconcat is True:
...
def rescale_forecasts_and_write_zarr(
    ds,
    scaler,
    zarr_fpath,
    chunks=None,
    ...
    for i in range(len(ds["forecast_reference_time"])):
        ...
        ds_tmp = (
            scaler.inverse_transform(ds_tmp)
            .swap_dims({"time": "leadtime"})
            .drop("time")
            .expand_dims("forecast_reference_time")
        )
